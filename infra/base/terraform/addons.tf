...
data "aws_iam_policy_document" "karpenter_controller_policy" {
  ...
  statement {
    ...
    resources = [
      "arn:${local.partition}:iam::${local.account_id}:role/karpenter-*",
      "arn:${local.partition}:iam::${local.account_id}:instance-profile/karpenter-*",
    ]
    effect = "Allow"
    sid    = "KarpenterControllerIAMPolicy"
  }
}
